...
//index page 
  ...
  const query = 'SELECT userid FROM users WHERE email = ? AND password = ?';
...
//main page
 router.get('/mainFetch', (req, res) => {
    ...
    const query = 'SELECT service_type, service_description FROM services';
...
router.get('/providerListFetch/:serviceType', (req, res) => {
     ...
     const query = 'SELECT name, short_description FROM service_provider WHERE job_type = ?';
...
router.post('/findServiceProviderId', (req, res) => {
  ...
  const query = 'SELECT serviceproviderid FROM service_provider WHERE name = ?';
...
router.get('/providerNameFetch/:providerId', (req, res) => {
   ...
   if (!req.session.userId) {
    ...
    let obj = [ProviderId];
    const query = 'SELECT name FROM service_provider WHERE serviceproviderid = ?';
    db.query(query,obj,(err, results) => {
      if (err) {
